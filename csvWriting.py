...
def get_numberofowners(leagueID, season):
...
def getteamid(player, leagueID, season):
...
def get_longest_bench(week, number_of_owners, leagueID, season):
    longest_bench_data = [0, 0]
    for i in range(1, number_of_owners + 1):
        page = requests.get(
            'https://fantasy.nfl.com/league/' + leagueID + '/history/' + season + '/teamgamecenter?teamId=' + str(
                i) + '&week=' + str(week))
...
def get_header(week, longest_bench_teamID, leagueID, season):
    url = "https://fantasy.nfl.com/league/" + leagueID + "/history/" + season + "/teamgamecenter?teamId=" + str(
        longest_bench_teamID) + "&week=" + str(week)
    page = requests.get(url)
    html = page.text
    page.close()
    soup = bs(html, 'html.parser')  # uses the page of the teamID with the longest bench to generate the header

    position_tags = [tag.find('span').text for tag in
                     soup.find('div', id='teamMatchupBoxScore').find('div', class_='teamWrap teamWrap-1').find_all('tr',
                                                                                                                   class_=re.compile(
                                                                                                                       'player-'))]
    ...
    header = ['Owner', 'Team', 'Rank'] + header + ['Total', 'Opponent_Owner', 'Opponent', 'Opponent Total']
...
def getrow(teamId, week, longest_bench, leagueID, season):
    # loads gamecenter page as soup
    page = requests.get(
        'https://fantasy.nfl.com/league/' + leagueID + '/history/' + season + '/teamgamecenter?teamId=' + teamId + '&week=' + week)
    soup = bs(page.text, 'html.parser')
    page.close()

    owner = soup.find('span', class_=re.compile('userName userId')).text  # username of the team owner
    team = soup.find('a', class_=re.compile('teamName teamId')).text  # team name
    ...
    player_totals = soup.find('div', id='teamMatchupBoxScore').find('div', class_='teamWrap teamWrap-1').find_all('td',
                                                                                                                  class_=re.compile(
                                                                                                                      "statTotal"))
    player_totals = [player.text for player in
                     player_totals]  # point totals for each player with indecies which correspond to that player's index in roster
    ...
    try:
        completed_row = [owner, team, rank] + rosterandtotals + [teamtotals[0].text,
                                                                 soup.find('div', class_='teamWrap teamWrap-2').find(
                                                                     'span', re.compile('userName userId')).text,
                                                                 soup.find('div', class_='teamWrap teamWrap-2').find(
                                                                     'a', re.compile('teamName teamId')).text,
                                                                 teamtotals[1].text]
    except:
        completed_row = [owner, team, rank] + rosterandtotals + [teamtotals[0].text, '-', '-']
...
for season in reversed(range(2015, 2023)):
    ...
    number_of_owners = get_numberofowners(leagueID=leagueID, season=season)

    for i in range(1,
                   season_length + 1):  # iterates through each week of the season, creating a new csv file every loop
        ...
        with open('./League-History/' + season + '/' + str(i) + '.csv', 'w', newline='') as f:
            ...
            for j in range(1, number_of_owners + 1):  # iterates through every team owner
                writer.writerow(getrow(str(j), str(i), longest_bench[0], leagueID=leagueID,
                                       season=season))  # writes a row for each owner in the csv
